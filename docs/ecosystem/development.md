...
Este é o guia detalhado, do manifesto ao resultado validado. Para uma primeira visão ou para converter uma automação existente, comece por [`quickstart.md`](quickstart.md). O protocolo normativo é [`protocol.md`](protocol.md), e a referência TypeScript é [`src/lib/plugin-contract.ts`](../../src/lib/plugin-contract.ts).

Plugins de automação ou mídia também devem aplicar integralmente as [convenções transversais de automação e mídia](automation-media-conventions.md), incluindo recibo de submissão, reconciliação, taxonomia de estados, materialização de CDN, options, logs, perfil, cancelamento e `promptPreview`.
...
Ele oferece `create`, `validate`, `test-contract`, `test-sandbox`, `fixture`, `report` e `check`, usa o mesmo validador do carregador real e nunca instala dependências nem executa scripts de instalação de terceiros. Há três templates em [`ecosystem/plugin-kit/templates`](../../ecosystem/plugin-kit/templates), uma [aula prática de 30 minutos](tutorial.md) e um [guia compacto para agentes de IA](ai-development.md).
...
O botão de exemplo preenche a pasta `Documentos\ContentFlow\Plugins\community-reference`, criada automaticamente pelo aplicativo desktop. Atualizar a página apenas relê os plugins conectados ou instalados; apagar a pasta de origem não desinstala uma cópia. Essa distinção evita perda acidental de um plugin já instalado.
...
Para traduzir textos fornecidos pelo próprio plugin, use o mapa opcional `localizations`. Mantenha o texto-base no manifesto e adicione apenas os campos visuais necessários por locale. O ContentFlow resolve `locale exato → idioma-base → texto-base`. Não duplique o manifesto inteiro por idioma e não traduza IDs, chaves de portas, valores de `enum`, aliases de perfil, nomes criados pelo usuário ou dados retornados pelo provedor.

Um mapa de localização pode traduzir plugin, capability, portas, ações por item e propriedades/opções estáticas do `blockConfigSchema`. As opções mantêm o mesmo `value`; apenas o `label` muda. Todos os alvos precisam existir no manifesto base, e um locale BCP 47 inválido faz o pacote falhar na validação antes de qualquer código do plugin ser executado.

Veja a fixture neutra em [`examples/plugin-localization.example.json`](examples/plugin-localization.example.json).

`blockConfigSchema` descreve opções escolhidas no método, como modelo, temperatura ou endpoint. Quando uma lista depende do estado real de uma conta ou perfil, declare `configurationOptions` na capability em vez de congelar um `enum`: o núcleo poderá solicitar as opções ao plugin por `configure/options`, usando em memória a mesma conexão local selecionada no bloco. `settingsSchema` descreve preferências locais reutilizadas entre métodos. Credenciais ficam apenas em `secretKeys`; chaves que habilitam recursos opcionais também entram em `optionalSecretKeys`, sempre como subconjunto, para que sua ausência não bloqueie capabilities independentes.

Para resultados compostos por itens manipuláveis, declare `itemActions` somente para ações realmente suportadas e use `services.publishPartial({ itemUpdates })` quando o provedor entregar resultados progressivamente. Informe também o `input` que originou cada slot para permitir regeneração precisa. A chave incremental é correlação local da tentativa e deve ser namespaced com `request.batch.itemId` em execuções item a item; IDs universais continuam sendo atribuídos pelo núcleo.
...
Se uma porta deve receber um documento, imagem, gravação, vídeo ou outro valor escolhido a cada execução, declare normalmente `acceptedTypes`, `multiple` e `presentation` no manifesto. No editor do Método, vincule essa porta com **Fornecer na execução**. O ContentFlow cria e valida a interface; o plugin recebe apenas o valor tipado em `request.inputs` e abre arquivos com `services.resolveInputFile()`.
