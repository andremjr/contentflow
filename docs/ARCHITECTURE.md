...
Automações de navegador podem cadastrar vários perfis de conta explicitamente preparados. Nos plugins oficiais de interface web da V1, cada plugin externo empacota sua própria extensão Manifest V3 e o runtime compatível, carregados automaticamente somente no navegador e no perfil dedicados. O núcleo não inclui extensão, Chromium, seletores ou adapters de provedor.
