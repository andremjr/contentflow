...
No Windows pessoal, a extensão companheira é instalada por perfil pela Chrome Web Store ou, durante desenvolvimento, por **Carregar sem compactação**. Uma CRX local só pode ser forçada para todos os perfis por política quando o dispositivo está vinculado a Active Directory, Azure AD ou Chrome Enterprise Core; o projeto não grava uma política que o Chrome recusará. A instalação administrativa é uma ferramenta externa e reversível da extensão, não uma responsabilidade do núcleo.
