...
function defaultProfilesBasePath() {
  return join(homedir(), ".contentflow", "meta-ai-browser-profiles");
}
...
async function attachExistingProviderPage(client, signal, activate = false) {
  const { targetInfos = [] } = await client.send("Target.getTargets");
...
async function clickMode(bridge, mode) {
  ...
  try {
    ...
    if (["image", "video"].includes(mode) && error?.code === "OUTPUT_VALIDATION_FAILED")
      return false;
...
async function clickSend(client, sessionId, bridge, signal, operationKey) {
  ...
  throw codedError("OUTPUT_VALIDATION_FAILED", "O Meta AI não confirmou o envio do prompt.", true);
}
...
async function configureProfile(request, services) {
  ...
  try {
    ...
    bridge = await attachContentFlowBridge({
      client,
      pageSessionId: authenticatedSessionId,
      pluginId: PLUGIN_ID,
...
export async function execute(request, services) {
  ...
  try {
    ...
    const attached = await attachChatGptPage(client, services.signal);
