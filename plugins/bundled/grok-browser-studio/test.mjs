...
test("isola contas por alias e porta", () => {
  ...
  assert.throws(() => __test.normalizeAccountProfile("../x"), /Perfil Grok/);
  assert.match(
    __test.profilePathFor({}, "canal-a").replaceAll("\\", "/"),
    /grok-browser-profiles\/canal-a$/,
  );
  assert.notEqual(__test.profilePort(9744, "canal-a"), __test.profilePort(9744, "canal-b"));
});
