            ...
            <aside className="lg:sticky lg:top-6 lg:h-fit">
              <div className="rounded-xl border border-border bg-card/60 p-5 backdrop-blur">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Resumo textual da sua configuração de pesquisa.
                </p>

                <Separator className="my-4" />

                <dl className="space-y-3 text-sm">
                  <PreviewRow label="Palavras-chave">
                    {keywords.length > 0 ? (
                  ...
                  <PreviewRow label="Idioma">
                    <span>
                      {activeLang?.flag} {activeLang?.name}
                    </span>
                  </PreviewRow>
                  <PreviewRow label="Duração">
                    {durationRange[0]}–{durationRange[1]} {durationUnit}
                  </PreviewRow>
                  <PreviewRow label="Postagem">
                    {dateFrom && dateTo
                      ? `${format(dateFrom, "d MMM", { locale: ptBR })} – ${format(dateTo, "d MMM yyyy", { locale: ptBR })}`
                      : "Qualquer data"}
                  </PreviewRow>
                  <PreviewRow label="Visualizações">
                    {viewMin != null ? formatCompact(viewMin) : "0"} –{" "}
                    {viewMax != null ? formatCompact(viewMax) : "∞"}
                  </PreviewRow>
                  <PreviewRow label="Comentários">
                    {commentMin != null ? formatCompact(commentMin) : "0"} –{" "}
                    {commentMax != null ? formatCompact(commentMax) : "∞"}
                  </PreviewRow>
                  <PreviewRow label="Tamanho do canal">
                    {formatCompact(channelSizeRange[0])} –{" "}
                    {formatCompact(channelSizeRange[1])} inscritos
                  </PreviewRow>
                  <PreviewRow label="Referências">
                    {refChannels.length > 0 ? (
                      <span>
                        {refChannels.length} canal
                        {refChannels.length > 1 ? "is" : ""}
                      </span>
                    ) : (
                      <Empty />
                    )}
                ...
                <div className="rounded-lg bg-secondary/40 p-3 text-xs text-muted-foreground">
                  ...
                  <p className="mt-1 leading-relaxed">
                    Configuração atual deve retornar{" "}
                    <span className="font-medium text-foreground">~842</span>{" "}
                    vídeos candidatos.
                  </p>
                </div>
